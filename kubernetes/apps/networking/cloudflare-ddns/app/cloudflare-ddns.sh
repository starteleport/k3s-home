...
desired_proxied="${CLOUDFLARE_PROXIED:-false}"
...
old_proxied=$(echo "$record_ipv4" | jq --raw-output '.result[0] | .proxied')

if [[ "${current_ipv4}" == "${old_ip4}" ]] && [[ "${desired_proxied}" == "${old_proxied}" ]]; then
    printf "%s - IP Address '%s' and proxied=%s have not changed" "$(date -u)" "${current_ipv4}" "${desired_proxied}"
    exit 0
fi
...
update_ipv4=$(curl -s -X PUT \
    ...
    -H "X-Auth-Key: ${CLOUDFLARE_APIKEY}" \
    -H "Content-Type: application/json" \
    --data "{\"id\":\"${zone_id}\",\"type\":\"A\",\"proxied\":${desired_proxied},\"name\":\"${CLOUDFLARE_RECORD_NAME}\",\"content\":\"${current_ipv4}\"}" \
)
if [[ "$(echo "$update_ipv4" | jq --raw-output '.success')" == "true" ]]; then
    printf "%s - Success - IP Address '%s' has been updated with proxied=%s" "$(date -u)" "${current_ipv4}" "${desired_proxied}"
    exit 0
else
